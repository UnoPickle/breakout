...
game_over_scene::game_over_scene(const uint64_t score) : m_title(
                                                         ...
                                                         m_navigation_restart(
                                                             static_cast<font_resource*>(g_resource_manager.
                                                                 get_resource("assets/Jersey10-Regular.ttf"))->get_font(
                                                                 NAV_TEXT_SIZE),
                                                             "Press '"  + std::string(SDL_GetKeyName(SDL_GetKeyFromScancode(RESTART_SCANCODE, SDL_KMOD_NONE, false))) + "' to restart", SCENE_TEXT_COLOR, {0, 0}),
                                                         m_navigation_menu(
                                                             static_cast<font_resource*>(g_resource_manager.
                                                                 get_resource("assets/Jersey10-Regular.ttf"))->get_font(
                                                                 NAV_TEXT_SIZE),
                                                             "Press '"  + std::string(SDL_GetKeyName(SDL_GetKeyFromScancode(MENU_SCANCODE, SDL_KMOD_NONE, false))) + "' for menu", SCENE_TEXT_COLOR, {0, 0})

{
    m_score.set_pos({
        static_cast<float>(breakout_defs::WINDOW_WIDTH) / 2 - m_score.get_surf().surface_object()->w / 2,
        static_cast<float>(breakout_defs::WINDOW_HEIGHT) / 2 - m_score.get_surf().surface_object()->h / 2 - BOTTOM_MARGIN
    });

    m_title.set_pos({
        static_cast<float>(breakout_defs::WINDOW_WIDTH) / 2 - m_title.get_surf().surface_object()->w / 2,
        m_score.get_pos().y - m_score.get_surf().surface_object()->h / 2 - m_title.get_surf().surface_object()->h / 2 - SCORE_TITLE_MARGIN
    });
    ...
    m_navigation_restart.set_pos({
        ...
        m_score.get_pos().y + m_score.get_surf().surface_object()->h / 2 + m_navigation_restart.get_surf().surface_object()->h / 2 + SCORE_TITLE_MARGIN
    });
